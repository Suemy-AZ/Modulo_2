...
Original file is located at
    https://colab.research.google.com/github/Suemy-AZ/Modulo2_CL/blob/main/M2_SinLib.ipynb
"""
...
"""Este modelo es muy malo ya que confunde la clase 1 con las demás clases y no es capaz de predecirla, por ello se implementará un modelo de orden 2 para intentar mejorar la clasificación ya que este modelo solo obtuvo un 0.3% de ajuste del modelo.
...
y_pred=[]
for x,x2  in zip(x,x2):
  probb= [h2(x,x2,theta12),h2(x,2,theta22),h2(x,x2,theta32)]
  y_p=probb.index(max(probb))+1
  y_pred.append(y_p)
...
"""Vemos que tiene un mejor desempeño que el de orden uno ya que tenemos un modelo con un 70% de ajuste y es capaz de clasificar en las 3 clases en buena manera."""